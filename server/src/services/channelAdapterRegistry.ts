...
import { WhatsAppChannelAdapter } from "../adapters/whatsAppChannelAdapter.js";
import { loadWhatsAppRuntimeConfig, type WhatsAppRuntimeConfig } from "../config/whatsappRuntimeConfig.js";
...
export type ChannelRegistryResolveOptions = Readonly<{ allowInboundFoundation?: boolean }>;
...
export class ChannelAdapterRegistry {
  ...
  resolve(channel: string, options: ChannelRegistryResolveOptions = {}): ChannelRegistryResolution {
    ...
    const inboundFoundationAllowed = entry.descriptor.status === "inbound-foundation" && options.allowInboundFoundation === true;
    if (entry.descriptor.status === "planned" || entry.descriptor.status === "foundation" || (entry.descriptor.status === "inbound-foundation" && !inboundFoundationAllowed)) return Object.freeze({ ok: false, channel, errorCode: "CHANNEL_NOT_IMPLEMENTED" });
    ...
    if ((!inboundFoundationAllowed && entry.descriptor.availability !== "available") || !entry.adapter) return Object.freeze({ ok: false, channel, errorCode: "CHANNEL_ADAPTER_UNAVAILABLE" });
...
export const createChannelAdapterRegistry = (whatsappConfig: WhatsAppRuntimeConfig = loadWhatsAppRuntimeConfig()): ChannelAdapterRegistry => new ChannelAdapterRegistry([
  ...
  { descriptor: descriptor("whatsapp"), adapter: new WhatsAppChannelAdapter(whatsappConfig) },
